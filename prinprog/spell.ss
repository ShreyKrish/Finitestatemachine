  ...
  (lambda (hashfunctionlist dict)
    (if (null? hashfunctionlist) '() ;;base case, return empty
        (append (map (car hashfunctionlist) dict)
                (Hasher (cdr hashfunctionlist) dict)))))
...
(define (key w)
  (define (key-iter w acc)
    (if (null? w)
        acc
        (key-iter (cdr w)
                  (+ (ctv (car w))
                     (* acc 29)))))
  (key-iter (reverse w) 5077))
...
(define gen-checker
  (lambda (hashfunctionlist dict)
      (let ((bit-vector (Hasher hashfunctionlist dict)))
        (lambda(w)
          (First bit-vector (Hasher hashfunctionlist (list w)))))))
